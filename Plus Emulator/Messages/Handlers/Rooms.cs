    ...
    partial class GameClientMessageHandler
    {
        ...
        internal void OnlineConfirmationEvent()
        {
            ...
            if (Session.GetHabbo().VIP && !Session.GetHabbo().GetBadgeComponent().HasBadge("VIP"))
            {
                ...
                Session.SendNotif("Obrigado por nos apoiar, aqui está o seu crachá VIP! ", "VIP Alert");
            }
            if (Session.GetHabbo().GetBadgeComponent().HasBadge("VIP") && !Session.GetHabbo().VIP)
            {
                ...
                Session.SendNotif("Seu crachá VIP foi removido porque você não é mais VIP.", "VIP Alert");
            }
            ...
            if (Session.GetHabbo().Rank >= 4 && !Session.GetHabbo().GetBadgeComponent().HasBadge("RSN"))
            {
                ...
                Session.SendNotif("Bem-vindo à equipe, aqui está seu crachá!", "Staff Alert");
            }
            if (Session.GetHabbo().GetBadgeComponent().HasBadge("RSN") && Session.GetHabbo().Rank <= 3)
            {
                ...
                Session.SendNotif("O crachá da sua equipe foi removido porque você não é mais funcionário.", "Staff Alert");
            }
        ...
        internal void SaveRoomData()
        {
            ...
            {
                Session.SendWhisper("Você não tem permissão para editar as configurações da sua sala. Entre em contato com um membro da equipe on-line para alterá-lo para você!");
                return;
            }
        ...
        internal void KickUser()
        {
            ...
            if (Session.GetHabbo().Rank >= 3 && !Session.GetHabbo().CurrentRoom.RoomData.Description.Contains("APARTMENT"))
            {
                ...
                else
                {
                    ...
                    Session.Shout("*Usa seus poderes divinos para banir o embaixador " + roomUserByHabbo.GetClient().GetHabbo().UserName + " for 60 minute(s)*");
                    return;
                }
            }
            else
        ...
        internal void GetRoomData2()
        {
            ...
            {
                Logging.LogException("Não é possível carregar o ID do quarto [" + Session.GetHabbo().LoadingRoom + "]" + ex);
            }
        }
